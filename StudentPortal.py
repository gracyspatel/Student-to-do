...
<LoginScreen>:
    ...
    MDLabel:
        id:label
        text: f"[font=Arial]Taskiva Login[/font]"
        markup: True
        pos_hint:{"center_y":.7}
        halign:"center"
        theme_text_color:"Custom"
...
def login_s():
    ...
    return S_id
def addcompTask(t11,t22,t33,t44):
    ...
    sql = "INSERT INTO cs_tasks (cs_name, cs_des,cs_date,cst_time) VALUES (%s, %s,%s,%s)"
    val = (t11,t22,t33,t44)
...
class ListItemWithCheckbox(ThreeLineAvatarIconListItem):
    ...
    def deletetask(self):
        ...
        mycursor = mydb.cursor()
        if(self.secondary_text=="Your Task"):
            sql= "SELECT * FROM s_tasks WHERE s_name =%s"
            adr=(self.text[4:],)
            ...
            sql="DELETE FROM s_tasks WHERE s_name=%s"
            adr=(self.text[4:],)
            mycursor.execute(sql,adr)
            mydb.commit()
        else:
        ...
        for x in myresult:
            if(x[0] not in save):
                ...
                self.ids.assigned.add_widget(
                    ListItemWithCheckbox(text=f"{c} : {x[0]}",secondary_text=f"Assigned Task",tertiary_text=f"Deadline :{x[2]}",icon=icons[47])
                    )
                c=c+1
        my=mydb.cursor()
        my.execute("SELECT s_name FROM s_tasks")
        myres=my.fetchall()
        for y in myres:
            if (y[0] not in save1):
                save1.append(y[0])
                self.ids.assigned.add_widget(
                    ListItemWithCheckbox(text=f"{c} : {y[0]}",secondary_text=f"Your Task",tertiary_text=f"Deadline :{x[2]}",icon=icons[47])
                    )
    ...
    def completedlist(self):
        ...
        for i in myresult:
            if(i[0] not in save2):
                ...
                c+=1
        mycursor.execute("SELECT * FROM cs_tasks")
        myresult = mycursor.fetchall()
        for i in myresult:
            if(i[0] not in save3):
                save3.append(i[0])
...
class AddScreen(Screen):
    ...
    def submitbtn(self):
        ...
        print(t)
        sttimeval=t
        sql = "INSERT INTO s_tasks (s_name, s_des,s_date,t_time) VALUES (%s, %s,%s,%s)"
